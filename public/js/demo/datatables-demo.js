...
$(document).ready(function() {
    ...
    if(parsePath("/","").includes("/notebook/view",0)){
        parsed = parsePath("/notebook/view/","");
        if(parsed != ""){
            itemNo = parsed.replace("/","");
        }else{
            itemNo = null;
        }
    }else{
...
console.log(parsePath("/",""));
    ...
    var receiptItemAggregatedTable
        ...
        $.ajax({
            ...
            "success": function(result) {
                ...
                $('.input-daterange input').each(function(index) {
                    if (index == 0) {
                        ...
                        $(this).datepicker().on('changeDate', function(e) {
                            ...
                            $('#receiptItemsAggregatedTable').DataTable().ajax.reload();
                        });
                    } else {
                ...
                var receiptsTable = $('#receiptsTable').DataTable({
                    ...
                    receiptItemAggregatedTable = $('#receiptItemsAggregatedTable').DataTable({
                        "lengthMenu": [10, 25, 50, 75, 100],
                        "pageLength": 10,
                        "responsive": true,
                        "processing": true,
                        ...
                        "ajax": {
                            // url: (base_url + parsePath("/RGMCS-Laravel/public/transaction/view/","/fetchtransactions")),
                            url: (base_url + "/fetchreceiptitemsaggregated"),
                            type: "post",
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
                        ...
                        "initComplete": function() {
                            var input = $('#receiptItemsAggregatedTable_filter input').unbind(),
                                self = this.api(),
                                $searchButton = $('<button class="btn btn-primary btn-sm ml-1">')
                                .text('Search')
                                .click(function() {
                            ...
                            $('#receiptItemsAggregatedTable_filter').append($searchButton, $clearButton);
                        }
                    }).on('draw.dt', function() {
                        $('.receipt-aggregate-info').on('click', function() {
                            console.log(this.id);
                        });
                    });
                    ...
                    $.ajax({
                        "method": "GET",
                        "url": (base_url + "/notebook/receipt-view"),
                        "data": {
